...
Tracing의 상세 설계와 구현 범위는 `src/agent_harness/tracing/TRACING.md`를 따른다.
...
`AgentRuntime`의 run execution event와 `ToolGateway`의 tool execution event를
동일한 `run_id`로 연결하여 in-memory recorder에 기록하는 흐름을 완료했다.
